...
function iconFor(loc: MapLocation): string {
  ...
  if (customMap[loc.id]) {
    return customMap[loc.id];
  }

  if (loc.type === 'ride') {
    return loc.category === 'Water' ? '🌊' : '🎢';
  }
  switch (loc.type) {
    case 'restaurant':
...
export default function MapCanvas({
  ...
  // Map click handler to select nearest marker on click
  useEffect(() => {
    ...
    const handleMapClick = (e: any) => {
      ...
      locations.forEach((l) => {
        const dist = Math.pow(l.lat - e.latlng.lat, 2) + Math.pow(l.lng - e.latlng.lng, 2);
        if (dist < bestDist) {
          bestDist = dist;
          best = l;
        }
      ...
      if (best) {
        onSelectRef.current((best as MapLocation).id);
      }
    };
  ...
  useEffect(() => {
    ...
    locations.forEach((loc) => {
      ...
      const bg =
        loc.type === 'ride'
          ? loc.status === 'open' ? '#f97316' : loc.status === 'maintenance' ? '#f59e0b' : '#94a3b8'
